#Paid Price: {order.paid_price} KES
...
#     print(f"✅ Order notification task triggered for Order ID: {order_id}")
...
#         order = Order.objects.select_related("buyer", "supplier").get(id=order_id)
#         buyer = order.buyer

#         # Assuming you have a reverse relation on order like `orderitems` from another model
#         # e.g. OrderItem with FK to Order and related_name='orderitems'
#         try:
#             items = order.orderitems.all()  # You MUST have related_name='orderitems' in your OrderItem model
#         except Exception:
#             items = []  # fallback if model missing

#         item_lines = []
#         for item in items:
#             item_lines.append(
#                 f"{item.product.name} - {item.quantity} x {item.price} KES"
#             )
...
# Order Date: {localtime(order.created_on).strftime('%Y-%m-%d %H:%M')}
# Status: {order.status}
